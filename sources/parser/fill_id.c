...
** Last update Fri Apr 15 14:51:26 2016 Thomas LE MOULLEC
*/
...
int             fill_id_pipe(t_data *data, int i)
{
  ...
  while (data->infos.tubes[j] != NULL)
    {
      if (my_strcmp_colon(data->infos.tubes[j], data->nodes[i].name) == SUCCESS)
        {
...
int             my_cut_id(t_data *data, int j)
{
  ...
  if (!(tmp = malloc(sizeof(*tmp) * (x - ptr + 1))))
    return (ERROR);
  x = 0;
  while (data->infos.tubes[j][ptr] != '\0')
    {
      tmp[x] = data->infos.tubes[j][ptr];
      x++;
      ptr++;
    }
  tmp[x] = '\0';
